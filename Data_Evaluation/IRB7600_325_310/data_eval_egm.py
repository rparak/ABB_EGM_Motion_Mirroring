...
def main():
    ...
    RESULT_PATH_NAME  = 'Test_Path_Experiment_1_T1'
    # Speed: 
    #   v50
    SPEED = 'v50'
    # Zone:
    ...
    if PLOT_DATA == 'Position':
        ...
        for i, (ax, desired, actual) in enumerate(zip(ax_vector, desired_position, actual_position)):
            ax.set_ylim([np.minimum.reduce([np.minimum.reduce(desired), np.minimum.reduce(actual)]) - 100.0, 
                         np.maximum.reduce([np.maximum.reduce(desired), np.maximum.reduce(actual)]) + 100.0])
